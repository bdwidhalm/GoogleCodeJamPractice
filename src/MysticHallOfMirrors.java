...
public class MysticHallOfMirrors
{
  ...
  void solve(Scanner sc, PrintWriter pw) {
      ...
      // create array with each row as an element
      for (int i=0; i<H; i++)
          map[i] = sc.next();

      // finding & setting starting position
      for (int i=0; i<H; i++)
          for (int j=0; j<W; j++)
              if (map[i].charAt(j) == 'X') {
                  stX = i + 0.5;
      ...
      // for the fog distance front to back -D to +D
  ...
  public static void main(String[] args) throws Exception {
      ...
      for (int caseNum=0; caseNum<caseCnt; caseNum++) {
          ...
          new MysticHallOfMirrors().solve(sc, pw);
      }
      pw.flush();
      pw.close();
      sc.close();
